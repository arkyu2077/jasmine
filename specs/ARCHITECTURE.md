# Jasmine 技术架构 — ARCHITECTURE.md

> **本文是 Jasmine 的系统架构真相源**。后续迭代请先读这里再下手 —— 模块边界、IPC 形状、运行时
...
Jasmine 是一个**桌面 app**（Tauri 2 + React + PixiJS），把 OpenAI 的 **Codex CLI**（用户本机已登录的
那份）当作 stateful 的生图 / 改图 agent。Jasmine 自己不做模型、不做编排、不卖 token —— 它只做
...
| **App（Jasmine）** | 制品 + 空间语义（文件身份、布局、文件夹同步、引用注入）|
...
2. **Board doc**（`<folder>/.jasmine/board.json`）—— 文件夹的空间投影：Placement / Annotation /
   布局。可重新构造，不持有制品。
3. **Session**（`<folder>/.jasmine/sessions/`）—— 连续对话；Codex 持有 `threadId`，App 仅镜像它
   + 消息时间线 JSONL。
...
| 图片协议 | 自定义 Jasmine 图片协议 | 前端只通过 `src/lib/jasmine-url.ts` 生成 URL；macOS/Linux/iOS 为 `jasmine://localhost/<boardId>/…`，Windows/Android 为 Tauri/WebView 需要的 `http://jasmine.localhost/<boardId>/…`；boardId 放 path + 路径规范化 + 防穿越 |
| Agent | **Codex `app-server`**（JSON-RPC over stdio）| 长驻进程 = stateful session，跟"每轮 respawn"完全不同 |
| 鉴权 | `~/.codex` ChatGPT 订阅 | **无 API key**；Jasmine 不接 OpenAI，能力来自用户已付费的 Codex |
...
- ❌ **自己接图像模型 / 编排多模型** → 能力交给 Codex，Jasmine 的价值在空间 UX。
...
```
┌────────────────────────────────────────────────────────────────────┐
│  Jasmine.app (Tauri host process)                                    │
│                                                                    │
│  ┌──────────────────────┐    ┌──────────────────────────────────┐  │
...
- **常规对外网络** = Codex sidecar。WebView 只读本地 Jasmine 图片协议，Rust 后端只读写本地文件（+
...
| 文件 | 行数 | 职责 |
|---|---:|---|
| `main.rs` | 5 | 二进制入口，调 `jasmine_lib::run()` |
| `lib.rs` | 171 | **Tauri Builder**：plugin（opener/dialog/notification/updater/process）+ 全局 state（BoardRegistry / CodexRegistry）+ 41 个 command 注册 + Jasmine 图片协议 + 窗口关闭→托盘逻辑 |
...
| `workspace.rs` | 117 | 全局工作区注册表（`~/.jasmine/boards.jsonl`）|
...
| `protocol.rs` | 82 | Jasmine 图片协议 URI 处理：从 BoardRegistry 路由到磁盘文件，兼容 Windows WebView2 `http://jasmine.localhost/...` 形态，做路径规范化 / 防穿越 |
| `paths.rs` | 100 | 文件系统布局：全局 `~/.jasmine/` + 每 Board `.jasmine/`，`JASMINE_HOME` 可覆盖（测试/便携）|

**系统服务**
| 文件 | 行数 | 职责 |
|---|---:|---|
| `config.rs` | 61 | `AppConfig`（proxy / close_to_tray / telemetry_opt_out / last_telemetry_date）+ `~/.jasmine/config.json` 原子 IO |
| `proxy.rs` | 819 | HTTP/SOCKS5 代理注入 —— 给 Codex sidecar spawn 时的 env 设 `HTTP(S)_PROXY` / `ALL_PROXY` / `NO_PROXY`；同时提供 Settings / AI 面板的轻量网络诊断 probe（WebView 只读本地 Jasmine 图片协议）|
...
user 发第一条
  ├─ App 渲染选中的图为 clean PNG + overlay PNG（写到 `<folder>/.jasmine/tmp/`）
...
| `permissionRequest` | Jasmine 自动 accept | workspace-write 沙箱兜底（与 Riff 故意 deny 相反）|
...
- Codex 用 `~/.codex` 的 ChatGPT 订阅，**Jasmine 完全不接 OpenAI**。
- 用户切换/登出 Codex → Jasmine 不感知，Codex sidecar 自己处理。
...
- `App.tsx` —— 顶层 layout：TopBar（标题 / 工作区开关 / AI 面板开关 / 设置 / 更新提示）+
  ToolBar（mark 工具 / 导入 / 选择 / 撤销 / 重做）+ JasmineCanvas（PixiJS）+ ChatPanel +
...
| 文件 | 行 | 职责 |
|---|---:|---|
| `JasmineCanvas.tsx` | 233 | React mount + 桥接 board↔scene；SelectionBar / CropOverlay / CanvasContextMenu 子组件 |
...
| `jasmine-url.ts` | Jasmine 图片 URL 单一出口；通过 Tauri `convertFileSrc` 推导平台协议 base，再按 `<boardId>/<rel-path>` 逐段编码 |
...
云能力**完全可选**：环境变量 `VITE_JASMINE_API_BASE` + `VITE_JASMINE_API_KEY` 没设的话整个 `src/services/cloud/`
...
### 8.1 全局（`~/.jasmine/`，由 `paths.rs::global_dir` 决定）
...
| `logs/jasmine.YYYY-MM-DD.log` | 日滚日志，保留 14 天 | `logging.rs` |

### 8.2 每 Board sidecar（`<folder>/.jasmine/`）
...
任何动 `.jasmine/` 布局或 `BoardDoc` 形状的改动都是 **versioned change**：
- 涨 `BoardDoc.version`；
...
**Windows + macOS Apple Silicon + macOS Intel**。Jasmine 不打包任何 sidecar / runtime（codex
...
| `build_dev.{sh,ps1}` | unsigned 调试包：mac 出 `Jasmine.app`，win 出 `jasmine.exe` |
...
- [ ] 涉及存储：动 `.jasmine/` 布局或 `BoardDoc` shape → versioned change + 迁移函数。
...
| 存储 | 注册表 / 全局名走 `~/.jasmine/`，画布 + 血缘走 folder sidecar |
| 鉴权 | `~/.codex` ChatGPT 订阅，无 API key |
| 引用 | v1 走文件路径，agent 自读，不挂传图 |
| 云 | 编译期开关，开源 fork 默认无云 |
| 协议 | AGPL-3.0（仓库：https://github.com/arkyu2077/jasmine）|

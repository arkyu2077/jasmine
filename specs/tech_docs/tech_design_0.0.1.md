...
- **纯 Rust GPU UI（Vello/egui/GPUI…）被否**：Vello 仍 alpha 且"图片是它最大短板"
  （无界数量图片正是 Jasmine 的工作负载）；egui 大图有秒级卡顿；GPUI 基本是单公司
...
图片经自定义协议 `jasmine://`（沿用 `riff://` 的路径规范化/防穿越）喂给画布，
...
> **OQ-2（已解决，2026-05-24）= 混合**：画布状态（Board doc + 标记 + **血缘** + session）放**文件夹内 `.jasmine/` sidecar**——自包含、可移植（拷贝/同步/换机不丢，类比 `.git`），且"位置即血缘"是关于文件的、留 folder 才语义完整。**身份/注册**（boardId、名字、路径、最近列表、app 侧偏好）放**全局 `~/.jasmine/workspaces.json`**，按 `boardId` keyed；folder 的 `meta.json.boardId` 当自报标记，移动/改名靠它重绑（不断指针）。**项目资源（未来：风格 prompt 等）**放 folder 内。

---

## 4. 存储布局（已实现，OQ-2 = 混合）

**两层 `.jasmine`，别混**（见 `src-tauri/src/paths.rs`）：

```
~/.jasmine/                      ← 全局：跨 Board 的 app 状态
...
└── .jasmine/                    ← Board sidecar（agent 看不到 / 不该改）
...
借鉴 Riff 的隔离原则：agent cwd = 文件夹本身（读输入图、写输出图），`.jasmine/`
...
**关键**：overlay-as-image 让 Jasmine 在能力层面 **runtime 无关**——只要 agent 能看图。
...
Jasmine 想要的"**stateful session，一个 Board 一个**"——比 Riff 的 CC 集成更顺。
...
`.jasmine/meta.json`）。`thread/resume` 失败且匹配 `no rollout found|thread not found`
→ 清掉 threadId、用 `thread/start` 重试一次，别丢用户消息。

**权限**：`thread/start` 带 `{approvalPolicy, sandbox}`；Jasmine 默认 **workspace-write +
非阻塞 approval**（编辑只在 Board 文件夹内）。Codex 会以 **server→client request** 形式
发审批/澄清请求 → Jasmine **接住并展示**（PRD §7.7 要 agent 能反问），用 `respond(id,…)` 回。
...
> **注意一个相反配置**：Riff v0.0.2 故意 deny `AskUserQuestion`；Jasmine **要允许并展示**
> agent 反问。
...
| ~~OQ-2~~ | ✅ 混合存储（2026-05-24）：画布状态 + 血缘 → folder `.jasmine/` sidecar（可移植）；身份/名字/注册表 → 全局 `~/.jasmine/workspaces.json`（按 boardId 重绑）；命名 `<origin>-<时间戳>` + `Asset.origin`。见 §3.1 / §4 |

...
- **是**：一套已落地的 design token + 组件外观规范。Jasmine 的 React chrome（侧栏 / 对话 / 工具栏 /
...
| J7 | **新增「画布交互色」**（§3.6，Jasmine 专属）：选区/手柄/标注用红 + 白 halo | 画布铺满任意图片，纯红描边会被红/暗图吞掉；加白 halo 保证任何底图上都可辨 |
...
- **底色 = 中性灰白**，浅色优先。大面积留白让内容（在 Jasmine 里＝图片）成为主角。
...
> 原参考标注 SF Pro Display（大标题）/ SF Pro Text（正文）+ 思源黑体；Jasmine 用系统等价（§4.1）。

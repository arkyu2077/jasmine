...
//!   2. a daily-rolling file at `~/.jasmine/logs/jasmine.log.YYYY-MM-DD` (kept 14
//!      days), written through a non-blocking worker.
//!
//! Default verbosity: `info` everywhere + `debug` for our own crate. Override
//! with `RUST_LOG` (e.g. `RUST_LOG=jasmine_lib=trace`).
//!
...
fn filter() -> EnvFilter {
    EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("info,jasmine_lib=debug"))
}
...
pub fn init() -> Option<WorkerGuard> {
    ...
    let file = Builder::new()
        ...
        .filename_prefix("jasmine")
        .filename_suffix("log")
        .max_log_files(14)
        .build(crate::paths::jasmine_logs_dir());
